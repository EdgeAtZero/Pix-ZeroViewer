...
class ShapeNestedScrollView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        val attr = context.obtainStyledAttributes(
            attrs,
            R.styleable.ShapeNestedScrollView,
            defStyleAttr,
            0
        )

        attr.getDimension(R.styleable.ShapeNestedScrollView_radius, -1F).also {
            if (it == -1F) {
                radius.fill(
                    attr.getDimension(R.styleable.ShapeNestedScrollView_radiusLeftTop, 0F),
                    0,
                    2
                )
                radius.fill(
                    attr.getDimension(R.styleable.ShapeNestedScrollView_radiusRightTop, 0F),
                    2,
                    4
                )
                radius.fill(
                    attr.getDimension(R.styleable.ShapeNestedScrollView_radiusRightBottom, 0F),
                    4,
                    6
                )
                radius.fill(
                    attr.getDimension(R.styleable.ShapeNestedScrollView_radiusLeftBottom, 0F),
                    6,
                    8
                )
            } else {
        ...
        circle = attr.getBoolean(R.styleable.ShapeNestedScrollView_circle, false)
